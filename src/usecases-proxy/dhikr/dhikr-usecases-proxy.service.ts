...
export class DhikrUsecasesProxyService {
  ...
  items() {
    return new GetDhikrItemsUsecase(this.dhikr).execute();
  }
  ...
  createLog(userId: string, data: any) {
    return new CreateDhikrLogUsecase(this.dhikr).execute(userId, data);
  }
  updateLog(userId: string, id: string, data: any) {
    return new UpdateDhikrLogUsecase(this.dhikr).execute(userId, id, data);
  }
}
