...
@Injectable()
export class LearningTypeormAdapter implements LearningPersistencePort {
  private readonly items: Repository<LearningItemTypeormEntity>;
  private readonly progress: Repository<UserLearningProgressTypeormEntity>;
  constructor(dataSource: DataSource) {
    this.items = dataSource.getRepository(LearningItemTypeormEntity);
    this.progress = dataSource.getRepository(UserLearningProgressTypeormEntity);
  }
  findItems() {
  ...
  findItemById(id: string) {
    return this.items.findOne({ where: { id } }) as any;
  }
  ...
  async updateProgress(id: string, data: any) {
    ...
    if (!existing) throw new NotFoundException('Learning progress not found');
    return this.progress.save({ ...existing, ...data });
  }
}
