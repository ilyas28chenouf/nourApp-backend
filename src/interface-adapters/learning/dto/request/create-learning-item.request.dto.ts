...
export class CreateLearningItemRequestDto { @ApiProperty() @IsUUID() learningItemId: string; @ApiProperty({ enum: LearningProgressStatus }) @IsEnum(LearningProgressStatus) status: LearningProgressStatus; @ApiPropertyOptional() @IsOptional() @IsInt() @Min(0) @Max(100) progressPercent?: number; @ApiPropertyOptional() @IsOptional() @IsDateString() lastReviewedAt?: string; }
