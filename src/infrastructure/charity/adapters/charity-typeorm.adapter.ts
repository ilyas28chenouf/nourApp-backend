...
export class CharityTypeormAdapter implements CharityPersistencePort {
  ...
  constructor(dataSource: DataSource) { this.repository = dataSource.getRepository(CharityLogTypeormEntity); }
  ...
  async update(id: string, data: any) { const existing = await this.repository.findOne({ where: { id } }); if (!existing) throw new NotFoundException('Charity log not found'); return this.repository.save({ ...existing, ...data }); }
