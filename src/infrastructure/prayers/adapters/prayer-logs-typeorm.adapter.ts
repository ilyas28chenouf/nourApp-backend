...
export class PrayerLogsTypeormAdapter implements PrayerLogsPersistencePort {
  ...
  constructor(dataSource: DataSource) { this.repository = dataSource.getRepository(PrayerLogTypeormEntity); }
  ...
  async update(id: string, data: any) { const existing = await this.repository.findOne({ where: { id } }); if (!existing) throw new NotFoundException('Prayer log not found'); return this.repository.save({ ...existing, ...data }); }
}
