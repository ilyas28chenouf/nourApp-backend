...
export class PrayerTimesTypeormAdapter implements PrayerTimesPersistencePort {
  ...
  constructor(dataSource: DataSource) { this.repository = dataSource.getRepository(PrayerTimeTypeormEntity); }
  findByUserAndDate(userId: string, prayerDate: string) { return this.repository.findOne({ where: { userId, prayerDate } }) as any; }
}
