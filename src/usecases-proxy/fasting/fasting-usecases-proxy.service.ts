...
export class FastingUsecasesProxyService {
  ...
  recommendedDays(month?: string) { return new GetRecommendedFastingDaysUsecase(this.fasting).execute(month); }
  logs(userId: string, from?: string, to?: string) { return new GetFastingLogsUsecase(this.fasting).execute(userId, from, to); }
  createLog(userId: string, data: any) { return new CreateFastingLogUsecase(this.fasting).execute(userId, data); }
  updateLog(userId: string, id: string, data: any) { return new UpdateFastingLogUsecase(this.fasting).execute(userId, id, data); }
  summary(userId: string, period: string) { return new GetFastingSummaryUsecase(this.fasting).execute(userId, period); }
}
