...
@Injectable()
export class FastingTypeormAdapter implements FastingPersistencePort {
  private readonly logs: Repository<FastingLogTypeormEntity>;
  private readonly days: Repository<FastingRecommendedDayTypeormEntity>;
  constructor(dataSource: DataSource) {
    this.logs = dataSource.getRepository(FastingLogTypeormEntity);
    this.days = dataSource.getRepository(FastingRecommendedDayTypeormEntity);
  }
  async findRecommendedDays(month?: string) {
    const days = await this.days.find({ order: { date: 'ASC' } });
    return month ? days.filter((d) => d.date.startsWith(month)) : days;
  }
